...
IOS_DATA = ROOT / "ios" / "HatInRing" / "HatInRing" / "HatInRingData"
ASSET_INDEX = ROOT / "assets" / "candidates" / "_index.json"
...
def _freshness_payload(today: date) -> dict:
    return {
        "mode": "pipeline",
        "as_of": today.isoformat(),
        "source": "Hat-in-Ring ingest pipeline",
        "build": "Generated by python -m hatring.pipeline",
    }
...
def _attach_ios_images(records: list[dict]) -> None:
    """Add bundled portrait paths to iOS data from the generated asset index."""
    ...
    try:
        ...
        return
    images = {
        item.get("id"): item.get("files", [None])[0]
        for item in index
        if isinstance(item, dict) and item.get("id") and item.get("files")
    }
    for record in records:
        image = images.get(record.get("id"))
        if image and (ROOT / image).exists():
            record["img"] = image


def _copy_ios_referenced_images(records: list[dict], target: Path) -> None:
    for record in records:
        ...
        if not source.exists():
            log.warning("ios: referenced portrait missing for %s: %s", record.get("id"), image)
            continue
...
def sync_ios_bundle(today: date, ios_data_dir: Path | None = None, include_assets: bool = False) -> None:
    """Copy current pipeline data into the native iOS bundle resources."""
    target = Path(ios_data_dir) if ios_data_dir else IOS_DATA
    target.mkdir(parents=True, exist_ok=True)

    required = [DATA / "candidates.json", DATA / "review_queue.json"]
    ...
    assets = ROOT / "assets" / "candidates"
    if include_assets and assets.exists():
        shutil.copytree(assets, target / "assets" / "candidates", dirs_exist_ok=True)

    log.info("ios: synced pipeline data -> %s", target)
...
def run(args):
    ...
    if args.sync_ios:
        sync_ios_bundle(
            today,
            Path(args.ios_data_dir) if args.ios_data_dir else None,
            include_assets=args.sync_ios_assets,
        )
...
def main(argv=None):
    ...
    p.add_argument("--sync-ios", action="store_true", help="copy refreshed data into the native iOS bundle")
    p.add_argument("--ios-data-dir", help="override native iOS HatInRingData resource directory")
    p.add_argument("--sync-ios-assets", action="store_true", help="also recopy candidate portrait assets into the iOS bundle")
    p.add_argument("-v", "--verbose", action="store_true")
    a = p.parse_args(argv)
    logging.basicConfig(
        level=logging.DEBUG if a.verbose else logging.INFO,
        format="%(levelname)s %(name)s: %(message)s")
    if not any([a.all, a.fec, a.news, a.build, a.sync_ios]):
        p.error("nothing to do: pass --all, --sync-ios, or some of --fec/--news/--build")
    run(a)
    return 0
